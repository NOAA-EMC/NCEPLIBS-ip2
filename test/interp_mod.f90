...
contains
  ...
  subroutine interp(grid, interp_opt)
    ...
    if (kind(output_data) == 8) then
       baseline_file = "./data/baseline_data/scalar/8_byte_bin/grid" // trim(grid) // ".opt" // interp_opt // ".bin_8"
    else
       baseline_file = "./data/baseline_data/scalar/4_byte_bin/grid" // trim(grid) // ".opt" // interp_opt // ".bin_4"
    endif
