...
contains
  SUBROUTINE POLATEV3(IPOPT,input_desc,output_desc, &
    ...
    ELSE
       ...
       CALL GDSWZD(output_desc,-1,MO,FILL,XPTS,YPTS, &
            RLON,RLAT,NO,CROT,SROT)
       IF(NO.EQ.0) IRET=3
    ENDIF
    CALL CHECK_GRIDS3V(input_desc, output_desc, SAME_GRIDI, SAME_GRIDO)
    IF(.NOT.SAME_GRIDI) THEN
       IF(MIX.NE.MI) THEN
          IF(MIX.GE.0) DEALLOCATE(XPTI,YPTI,RLOI,RLAI,CROI,SROI)
          ALLOCATE(XPTI(MI),YPTI(MI),RLOI(MI),RLAI(MI),CROI(MI),SROI(MI))
          MIX=MI
       ENDIF
       CALL GDSWZD(input_desc, 0,MI,FILL,XPTI,YPTI, &
    ...
    DO NB=1,NB3
       ...
       IF(WB.NE.0) THEN
          ...
          ENDDO
          !$OMP END PARALLEL DO
          CALL GDSWZD(output_desc, 1,NO,FILL,XPTB,YPTB, &
               RLOB,RLAB,NV)
          CALL GDSWZD(input_desc,-1,NO,FILL,XPTB,YPTB, &
               RLOB,RLAB,NV)
  ...
  ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  SUBROUTINE CHECK_GRIDS3V(input_desc, output_desc, SAME_GRIDI, SAME_GRIDO)
    !$$$  SUBPROGRAM DOCUMENTATION BLOCK
    !
